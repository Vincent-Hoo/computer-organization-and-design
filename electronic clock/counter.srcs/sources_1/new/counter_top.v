...
module electronic_clock #(parameter N1 = 9, N2 = 5, N3 = 2, N4 = 3) (
    ...
    //两个消抖模块
    button ab(.button_in(addOne), .clk(clk_10Hz), .button_out(add));
    button cb(.button_in(carryOut), .clk(clk_10Hz), .button_out(carry));
    ...
    begin
        ...
        begin
            if ( cnt6 > alarm_clock_cnt6 )
                alarm_clock_arrive_time <= 1;
            else if( cnt6 < alarm_clock_cnt6 )
                alarm_clock_arrive_time <= 0;           
            else if( cnt6 == alarm_clock_cnt6 )
            begin
                if( cnt5 > alarm_clock_cnt5 )
                    alarm_clock_arrive_time <= 1;
                else if( cnt5 < alarm_clock_cnt5 )
                    alarm_clock_arrive_time <= 0;
                else if( cnt5 == alarm_clock_cnt5 )
                begin
                    if( cnt4 > alarm_clock_cnt4 )
                        alarm_clock_arrive_time <= 1;
                    else if( cnt4 < alarm_clock_cnt4 )
                        alarm_clock_arrive_time <= 0;
                    else if( cnt4 == alarm_clock_cnt4 )
                    begin
                        if( cnt3 > alarm_clock_cnt3 )
                            alarm_clock_arrive_time <= 1;
                        else if( cnt3 < alarm_clock_cnt3 )
                            alarm_clock_arrive_time <= 0;
                        else if( cnt3 == alarm_clock_cnt3 )
                        begin
                            if( cnt2 > alarm_clock_cnt2 )
                                alarm_clock_arrive_time <= 1;
                            else if( cnt2 < alarm_clock_cnt2 )
                                alarm_clock_arrive_time <= 0;
                            else if( cnt2 == alarm_clock_cnt2 )
                            begin
                                if( cnt1 > alarm_clock_cnt1 || cnt1 == alarm_clock_cnt1 )
                                    alarm_clock_arrive_time <= 1;
                                else if( cnt1 < alarm_clock_cnt1 )
                                    alarm_clock_arrive_time <= 0;
                            end
                        end
                    end
...
module counter #(parameter N1 = 9, N2 = 5) (
    ...
    button pb(.button_in(pause), .clk(clk_10Hz), .button_out(pau));
...
module counter_top(
    ...
    wire clk_1Hz,clk_10Hz,clk_60Hz;
    wire [15:0] cnter ;
    wire [15:0] clock ;
    reg [15:0] x;
    ...
    counter cnt(.clk_10Hz(clk_10Hz), .clk_60Hz(clk_60Hz), .clr(clr), .shift(shift), .pause(pause), .x(cnter));
     
    electronic_clock c(.one_second_clk(clk_1Hz), .clk_10Hz(clk_10Hz), .alarm_clock_mode(alarm_clock_mode), .set(set), .shift(shift), .addOne(addOne), .carryOut(carryOut), .x(clock), .L0(L0), .L1(L1), .L2(L2), .L3(L3), .L4(L4), .L5(L5), .L6(L6), .L7(L7), .L8(L8), .L9(L9), .L10(L10), .L11(L11), .L12(L12), .L13(L13), .L14(L14), .L15(L15));
